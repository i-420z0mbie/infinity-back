...
export default function Home({ navigation, route }) {
  ...
  // Initialize from Splash's params
  const [properties, setProperties] = useState(initialData.properties);
  const [explore, setExplore] = useState(initialData.explore);
  const [typeTabs, setTypeTabs] = useState(initialData.typeTabs);
  const [username, setUsername] = useState(initialData.username);
  const [favoritesMap, setFavoritesMap] = useState(initialData.favoritesMap);
  ...
  const recs = /* filter by typeTabs */ activeType
      ...
      <FlatList
        ...
        ListHeaderComponent={() => (
          <View style={styles.container}>
            {/* your header, featured, tabs, etc. all unchanged */}
          </View>
        )}
        data={explore}
        numColumns={2}
