  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        console.warn('Splash loadData', e);
        setData({ properties: [], explore: [], typeTabs: [], username: '', favoritesMap: {} });
      } finally {
        // **Replace** the splash with the Main route
        router.replace('Main');
      }
    })();
  }, []);
...
const styles = StyleSheet.create({
  ...
  glow: {
    ...
    ...Platform.select({ ios:{shadowColor:'#FFD700',shadowOffset:{0:0},shadowOpacity:0.8,shadowRadius:40}, android:{elevation:30} }),
  },
  logoContainer: {
    ...Platform.select({ ios:{shadowColor:'#FFD700',shadowOffset:{0:0},shadowOpacity:0.7,shadowRadius:15}, android:{elevation:20,overflow:'hidden',borderRadius:5} }),
  },
  title: {
    color:'#FFD700', fontFamily:'LeckerliOne-Regular', textAlign:'center',
    letterSpacing:1.5, textShadowColor:'rgba(255,215,0,0.4)',
    textShadowOffset:{0:0}, textShadowRadius:10, paddingHorizontal:20
  },
