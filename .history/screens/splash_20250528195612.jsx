...
import DataContext from '../app/DataContext';  // default import
...
export default function Splash() {
  ...
  // 1) Kick off animations on mount
  useEffect(() => {
    Animated.sequence([
      Animated.parallel([
  ...
  // 2) Independently load data and navigate once done
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const resProps = await api.get('main/properties/');
        const verified = resProps.data.filter(p => p.is_verified);
          ...
          await Promise.all(
            verified.map(async (p) => {
              try {
                ...
                favoritesMap[p.id] = { liked, favId: liked ? favRes.data[0].id : null };
              } catch {
    ...
    <View style={styles.container}>
      ...
      <Animated.View
        style={[
          ...
          { opacity: fadeAnim, transform: [{ scale: scaleAnim }], marginBottom: isSmallScreen ? 15 : 25 },
        ]}
      >
        <Image
