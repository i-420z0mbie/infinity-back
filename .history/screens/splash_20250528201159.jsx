...
export default function Splash() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const { data: all } = await api.get('main/properties/');
        const verified = all.filter(p => p.is_verified);
        ...
        console.warn('Splash load error', e);
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <Animated.View style={[styles.logoContainer, { opacity: fadeAnim, transform: [{ scale: scaleAnim }], marginBottom: isSmall ? 15 : 25 }]}>
        <Image source={require('../assets/images/casaz-logo.png')}
               style={{ width: logoSize, height: logoSize }} resizeMode="contain" />
      </Animated.View>
      <Animated.Text style={[ styles.title, { opacity: textFade, transform: [{ translateY: textPos }], marginBottom: isSmall ? 20 : 35 } ]}>
...
const styles = StyleSheet.create({
  ...
  glow: { position:'absolute', width:'70%', aspectRatio:1, borderRadius:200, backgroundColor:'rgba(212,175,55,0.25)',
    ...Platform.select({ ios:{shadowColor:'#FFD700',shadowOffset:{width:0,height:0},shadowOpacity:0.8,shadowRadius:40}, android:{elevation:30} }),
  },
  logoContainer: { ...Platform.select({ ios:{shadowColor:'#FFD700',shadowOffset:{width:0,height:0},shadowOpacity:0.7,shadowRadius:15}, android:{elevation:20,overflow:'hidden',borderRadius:5} }) },
  title: { color:'#FFD700', fontFamily:'LeckerliOne-Regular', textAlign:'center', letterSpacing:1.5,
           textShadowColor:'rgba(255,215,0,0.4)', textShadowOffset:{width:0,height:0}, textShadowRadius:10, paddingHorizontal:20 },
